...
ClientController::ClientController(PeerInfo *_peer, Defines *_defines, FragmentRepository *_fragmentRepository,
                                   int _clientId, int _remotePeerId, int _clientFileDescriptor) :
        peer(_peer), defines(_defines), fragmentRepository(_fragmentRepository), clientId(_clientId),
        remotePeerId(_remotePeerId), clientFileDescriptor(_clientFileDescriptor) {}
...
void ClientController::Startup() {
    if (peer->IsInteresting(clientId, remotePeerId)) {
        ...
        if (peer->HasFile(clientId)) {
            ChokeOrUnchokePeers(peer->GetInterestedTable(clientId), defines->GetNumberOfPreferredNeighbors(), clientId,
                                remotePeerId, peer);
        } else {
            ChokeOrUnchokePeers(peer->GetInterestedTable(clientId), defines->GetNumberOfPreferredNeighbors(), clientId,
                                remotePeerId, peer);
...
void ClientController::ChokeOrUnchokePeers(std::map<int, bool> interestedTable, size_t preferableNumber, int clientId,
                                           int remotePeerId, PeerInfo *peer) {
    ...
    for (int unchokedPeerId : unchokedPeers) {
        if (std::find(preferableNeighbors.begin(), preferableNeighbors.end(), unchokedPeerId) ==
            preferableNeighbors.end()) {
            ...
            for (size_t j = 1001; j < peer->GetPeerNetworkSize() + 1001; j++) {
                ...
                if (clientId != (int) j && peer->GetPieceStatus(j, i)) {
                    ...
                    uint32_t length = Utility::UintToCharVector(
                            std::vector<char>(response.begin(), response.begin() + 4));
                    response.resize(length + 4);

                    fragmentRepository->PlaceFragment(clientId, i, response);

                    SendNotInterestedMessage(peer, clientId, i);
                    break;
                }
            }
        }
...
void ClientController::SendNotInterestedMessage(PeerInfo *peer, int clientId, int i) {
